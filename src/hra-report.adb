package body HRA.Report is
   ...
   function Has_Account_Activity_Through
     (L       : Ledger.Ledger;
      Acc     : Account.Account;
      Through : HRA.Dates.Date) return Boolean
   is
   begin
      for Tx of L.Transactions loop
   ...
   function Has_Account_Activity_In
     (L      : Ledger.Ledger;
      Acc    : Account.Account;
      Period : HRA.Dates.Closed_Period) return Boolean
   is
   begin
      for Tx of L.Transactions loop
         if HRA.Dates.Contains (Period, Tx.Date) then
            for P of Tx.Postings loop
               if P.Acc = Acc then
                  return True;
               end if;
   ...
   function Generate_Trial_Balance_As_Of
     (L          : Ledger.Ledger;
      As_Of_Date : HRA.Dates.Date) return Trial_Balance
   is
      TB : Trial_Balance;
   begin
   ...
   function Generate_Profit_And_Loss_Period
     (L      : Ledger.Ledger;
      Period : HRA.Dates.Closed_Period) return Profit_And_Loss
   is
      PL : Profit_And_Loss;
   begin
   ...
   function Generate_Profit_And_Loss_Through
     (L       : Ledger.Ledger;
      Through : HRA.Dates.Date) return Profit_And_Loss
   is
      PL : Profit_And_Loss;
   begin
      PL.Total_Income   := Empty_Balance;
   ...
   function Generate_Balance_Sheet_As_Of
     (L          : Ledger.Ledger;
      As_Of_Date : HRA.Dates.Date) return Balance_Sheet
   is
      BS : Balance_Sheet;
