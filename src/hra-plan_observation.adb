...
package body HRA.Plan_Observation is
   ...
   function Admit_Plan_Identities
     (Plan_Ledger   : HRA.Ledger.Ledger;
      ...
      Diag          : out Admission_Diagnostic) return Boolean
   is
      Output : HRA.Plan.Plan_Id_Universe :=
   ...
   begin
      ...
      for I in 1 .. Natural (Plan_Ledger.Transactions.Length) loop
         ...
         begin
            ...
            if HRA.Plan.Contains (Output, PID) then
               Diag :=
                 (Status      => Duplicate_Plan_Id,
                  Line_Number => Plan_Meta.Line_Number,
                  Plan_Id     => To_Unbounded_String (HRA.Plan.Text (PID)),
                  Message     => To_Unbounded_String
                    ("plan-id identifies more than one transaction"));
               return False;
            end if;

            HRA.Plan.Include (Output, PID);
         end;
      end loop;
   ...
   begin
      Result := HRA.Plan.Empty_Plan_Id_Universe;
   ...
   is
      ...
      function Completion_Exists (ID : HRA.Plan.Plan_Id) return Boolean is
      begin
            ...
            if Count > 1 then
               ...
               begin
                  ...
                  then
                     ...
                     return False;
                  elsif not HRA.Plan.Contains (Known_Plans, PID) then
                     Fail
   ...
   is
      ...
      function Visible_Completion_Index
        (ID : HRA.Plan.Plan_Id) return Natural
      is
      begin
      ...
      function Supersession_Cycle_From (Start : Positive) return Boolean is
         Current : HRA.Plan.Plan_Id := Plans.Element (Start).ID;
      begin
   ...
   begin
      ...
      for I in 1 .. Natural (Plan_Ledger.Transactions.Length) loop
         ...
         begin
            ...
            P.Superseded_By := HRA.Plan.Null_Plan_Id;

            if Cancel_Count = 1 then
               declare
                  Lifecycle_Date : HRA.Dates.Date;
      ...
      for I in 1 .. Natural (Plans.Length) loop
         ...
         then
            Fail
              (Supersession_Cycle,
               0,
               HRA.Plan.Text (Plans.Element (I).ID),
      ...
      begin
         for P of Plans loop
            HRA.Plan.Include (Known_Plans, P.ID);
         end loop;
