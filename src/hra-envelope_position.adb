...
package body HRA.Envelope_Position is
   ...
   function Status_For
     (Status : HRA.Proof_Money_Bridge.Bridge_Status) return Observe_Status
   is
   begin
      case Status is
         when HRA.Proof_Money_Bridge.Success =>
            return Success;
         when HRA.Proof_Money_Bridge.Out_Of_Proof_Input_Range =>
            return Proof_Input_Out_Of_Range;
         when HRA.Proof_Money_Bridge.Out_Of_Money_Output_Range =>
            return Proof_Output_Out_Of_Range;
         when HRA.Proof_Money_Bridge.Non_Exact_Conversion =>
   ...
   is
      ...
      function Admit_Input
        (Value     : Balance;
         ...
         Role      : Value_Role;
         Quanta    : out HRA.Proof_Core.Atomic_Quanta) return Boolean
      is
         Bridge_Status : HRA.Proof_Money_Bridge.Bridge_Status;
      begin
         if HRA.Proof_Money_Bridge.Balance_To_Atomic_Quanta
      ...
      is
         ...
         Singleton     : Balance;
      begin
         if not HRA.Proof_Money_Bridge.To_Singleton_Balance
   ...
   begin
      ...
      for Commodity of Coordinates loop
         declare
            Ent_Q   : HRA.Proof_Core.Atomic_Quanta;
            Cons_Q  : HRA.Proof_Core.Atomic_Quanta;
            Ful_Q   : HRA.Proof_Core.Atomic_Quanta;
            Plan_Q  : HRA.Proof_Core.Atomic_Quanta;
         begin
            if not Admit_Input
            ...
            declare
               Proof_Result : constant HRA.Proof_Core.Envelope_Result :=
                 HRA.Proof_Core.Evaluate_Envelope
                   ((Entitlement     => Ent_Q,
   ...
   begin
      ...
      Evidence :=
        (Entitlement          => Entitlement,
         ...
         Net_Consumption      =>
           HRA.Envelope_Consumption.Net_Consumption (Consumption),
         Fulfillment_Applied  => Fulfillment.Applied,
         Fulfillment_Reversed => Fulfillment.Reversed,
         Net_Fulfillment      =>
           HRA.Envelope_Fulfillment.Net_Fulfillment (Fulfillment),
            ...
            if not Add_Current_Position
              (Output,
               ...
               HRA.Envelope_Consumption.Consumption_For
                 (Consumption, Env),
               HRA.Envelope_Fulfillment.Empty_Amounts,
               Empty_Balance,
               Diag)
            then
               Result := Output;
            ...
            if not Add_Current_Position
              (Output,
               ...
               HRA.Envelope_Fulfillment.Fulfillment_For
                 (Fulfillment, Env),
               HRA.Envelope_Commitment.Commitment_For (Commitment, Env),
               Diag)
            then
               Result := Output;
               return False;
