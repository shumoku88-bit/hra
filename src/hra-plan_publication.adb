...
package body HRA.Plan_Publication is
   ...
   is
      Graph             : constant HRA.Plan_Graph_Admission.Candidate_Graph :=
        HRA.Plan_Account_Admission.Graph_Of (Candidate);
   ...
   begin
      ...
      if Source_Count = 0 then
         ...
         Diag.Message := To_Unbounded_String
           ("Account-qualified Plan graph has no retained source witness");
         return False;
      end if;

      declare
         Root_Witness : constant HRA.Journal_Loader.Source_Observation :=
           HRA.Plan_Graph_Admission.Source_At (Graph, 1);
      begin
         ...
         then
            Diag.Status := Invalid_Qualified_Source_Witness;
            Diag.Message := To_Unbounded_String
              ("Account-qualified Plan graph root witness is inconsistent");
            return False;
         end if;
      end;
      ...
      for Source_Index in 2 .. Source_Count loop
         declare
            Source : constant HRA.Journal_Loader.Source_Observation :=
              HRA.Plan_Graph_Admission.Source_At (Graph, Source_Index);
         begin
            if Source.Kind /= HRA.Journal_Loader.Included_File then
               Diag.Status := Invalid_Qualified_Source_Witness;
               Diag.Message := To_Unbounded_String
                 ("Account-qualified Plan graph contains a non-include guard witness");
               return False;
            end if;
      ...
      if not HRA.Writer.Atomic_Publish_Journal_Guarded
        (Target_Path => HRA.Plan_Root_Candidate.Root_Path_Of (Root),
         Expected    => HRA.Writer.Make_Expected_Source
           (HRA.Plan_Root_Candidate.Observed_Text (Root)),
   ...
   is
      ...
      Source_Count : constant Natural :=
        HRA.Plan_Graph_Admission.Source_Count (Graph);
      Premises     : HRA.Writer.Source_Premise_Array (1 .. Source_Count);
   begin
      Premises (1) :=
        HRA.Writer.Make_Source_Premise
          (Path     => HRA.Plan_Root_Candidate.Root_Path_Of (Root),
           Expected => HRA.Writer.Make_Expected_Source
             (HRA.Plan_Root_Candidate.Text (Root)));
