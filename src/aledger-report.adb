...
package body ALedger.Report is
   ...
   procedure Collect_Account_Balances
     (L    : Ledger.Ledger;
   ...
   begin
      Collect_Account_Balances (L, Map);
      Cursor := Map.First;
      while Account_Balance_Maps.Has_Element (Cursor) loop
         ...
         begin
            TB.Lines.Append ((Acc => Acc, Bal => Bal));
      ...
      while Account_Balance_Maps.Has_Element (Cursor) loop
         ...
         begin
            if Account_Type_For (L.Registry, Acc, Cat) then
               if Cat = Income then
                  --  Income credit balance is normalized to positive presentation
                  declare
                     Norm_Bal : constant Balance := Negate_Balance (Bal);
                  begin
                     PL.Income_Lines.Append ((Acc => Acc, Bal => Norm_Bal));
                     Tot_Inc := Add_Balance (Tot_Inc, Norm_Bal);
                  end;
               elsif Cat = Expense then
                  PL.Expense_Lines.Append ((Acc => Acc, Bal => Bal));
   ...
   begin
      ...
      while Account_Balance_Maps.Has_Element (Cursor) loop
         ...
         begin
            if Account_Type_For (L.Registry, Acc, Cat) then
               if Cat = Asset then
                  BS.Asset_Lines.Append ((Acc => Acc, Bal => Bal));
                  Tot_Ast := Add_Balance (Tot_Ast, Bal);
               elsif Cat = Liability then
                  declare
                     Norm_Bal : constant Balance := Negate_Balance (Bal);
                  begin
                     BS.Liability_Lines.Append ((Acc => Acc, Bal => Norm_Bal));
                  ...
                  declare
                     Norm_Bal : constant Balance := Negate_Balance (Bal);
                  begin
                     BS.Equity_Lines.Append ((Acc => Acc, Bal => Norm_Bal));
