...
package body HRA.Actual_Admission is
   ...
   function Effects_Are_Inverse
     (Left, Right : HRA.Ledger.Transaction) return Boolean
   is
      function Coordinate_Is_Inverse
        (Acc : HRA.Account.Account; Comm : Commodity) return Boolean
      is
         L_Total : Quantity;
         R_Total : Quantity;
         Sum     : Quantity;
   ...
   begin
      ...
      for I in 1 .. Natural (Actual_Ledger.Transactions.Length) loop
         declare
            ...
            Normalized_Tx : HRA.Ledger.Transaction := Tx;
         ...
         begin
            ...
            if Plan_Count = 1 then
               ...
               if not Has_Event then
                  if not Create_Actual_Id
                    ("plan-completion-" & HRA.Plan.Text (PID),
