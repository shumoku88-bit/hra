...
package body HRA.Envelope_Consumption is
   ...
   is
      ...
      function Resolve_Root_Date
        (Event_Id : String;
         Own_Date : HRA.Dates.Date) return HRA.Dates.Date
      is
      ...
      function Date_In_Scope (Date : HRA.Dates.Date) return Boolean is
      begin
         case Scope.Kind is
   ...
   begin
      ...
      for Tx of L.Transactions loop
         if Date_In_Scope (Tx.Date) then
            declare
               Ev_Id     : constant String := To_String (Tx.Event_ID);
               Root_Date : constant HRA.Dates.Date :=
   ...
   function Observe_Consumption
     (L            : Ledger.Ledger;
      Routing      : Envelope_Routing.Routing_History;
      Through_Date : HRA.Dates.Date) return Envelope_Consumption
   is
   begin
      return Observe_Internal
        (L,
         Routing,
         (Kind => HRA.Envelope_Consumption.Through_Date,
