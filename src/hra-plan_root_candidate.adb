...
package body HRA.Plan_Root_Candidate is
   ...
   is
      ...
      Block_Text         : constant String := HRA.Plan_Candidate.Text (Block);
   begin
      Candidate :=
      ...
      then
         ...
         Diag.Message := To_Unbounded_String
           ("existing Plan root source is not admitted by Journal syntax");
         return False;
      end if;
      ...
      then
         ...
         Diag.Message := To_Unbounded_String
           ("existing Plan root source does not retain aligned evidence");
         return False;
      end if;
      ...
      then
         ...
         Diag.Journal := Journal_Diag;
         Diag.Message := To_Unbounded_String
           ("Plan root candidate is not admitted by Journal syntax");
         return False;
      end if;
      ...
      then
         ...
         Diag.Evidence := Evidence_Diag;
         Diag.Message := To_Unbounded_String
           ("Plan root candidate does not retain aligned source evidence");
         return False;
      end if;

      --  Candidate_Block is private and already qualified by Plan_Candidate,
      ...
      Journal_Diag := Empty_Journal_Diagnostic;
      if not HRA.Journal.Parse_Journal_Text
        (Block_Text, "<plan-root-block>", Block_Ledger, Journal_Diag)
      then
         Diag.Status := Semantic_Roundtrip_Failed;
         Diag.Journal := Journal_Diag;
         Diag.Message := To_Unbounded_String
           ("qualified Plan block no longer admits as one root-local suffix");
         return False;
      end if;
      ...
      then
         ...
         Diag.Evidence := Evidence_Diag;
         Diag.Message := To_Unbounded_String
           ("qualified Plan block lost root-local source evidence");
         return False;
      end if;
      ...
      begin
         ...
         then
            Diag.Status := Semantic_Roundtrip_Failed;
            Diag.Message := To_Unbounded_String
              ("Plan root candidate did not add exactly one direct root transaction");
            return False;
         end if;
         ...
         for I in 1 .. Existing_Count loop
            ...
            then
               Diag.Status := Semantic_Roundtrip_Failed;
               Diag.Message := To_Unbounded_String
                 ("Plan root candidate changed existing direct transaction meaning");
               return False;
            end if;
         end loop;
         ...
         then
            Diag.Status := Semantic_Roundtrip_Failed;
            Diag.Message := To_Unbounded_String
              ("Plan root candidate changed appended block transaction meaning");
            return False;
         end if;
         ...
         then
            Diag.Status := Semantic_Roundtrip_Failed;
            Diag.Message := To_Unbounded_String
              ("Plan root candidate changed appended block evidence meaning");
            return False;
         end if;
      end;
