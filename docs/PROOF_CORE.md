...
hraでは、parser、filesystem、Report、TUIを全面SPARK化しない。ordinary Adaがsource-specific admissionとcross-source validationを所有し、その結果から作られた小さなbounded factsだけをpure arithmetic kernelへ渡す。

```text
canonical 8-source exact observation
  -> ordinary Ada admission / validation
  -> normalized proof facts
  -> HRA.Proof_Core (SPARK_Mode)
...
- `HRA.Proof_Money_Bridge`: `HRA.Money.Quantity` / `Balance`とproof quanta（`10^-8`）のexact checked conversion
...
`HRA.Proof_Core`は1単位を`10^-8`とするsigned integer quantaを使う。
...
- production Envelope Remaining / Headroom: `HRA.Envelope_Position` 経由で `HRA.Proof_Core.Evaluate_Envelope` へ接続完了（Phase C）
- multi-Commodity coordinate evaluation: 各 Envelope 内で 4 入力（Entitlement, Net Consumption, Net Fulfillment, Plan Commitment）の Commodity 座標の union を構成し、各 Commodity 座標ごとに `HRA.Proof_Money_Bridge` を介して `Proof_Core.Evaluate_Envelope` で評価（接続完了）
...
- `HRA.Proof_Money_Bridge`がowner
- `HRA.Money.Quantity`と`HRA.Proof_Core` quanta（`10^-8`）のexact checked conversion
...
- `HRA.Backing_Policy` は Remaining/Headroom の計算責任を完全に手放し、`Envelope_Position.Observation` を入力として消費するのみ。要求 Envelope の Position 欠損時は fail-loud
