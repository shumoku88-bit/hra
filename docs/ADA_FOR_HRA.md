...
- **[HRA]** このrepositoryで選ぶ設計方針
- **[Current]** 現在の実装形。長期的な規則ではない

`[HRA]` と `[Current]` はAdaそのものではない。
...
**[HRA]** 新しい抽象化を作る前に、上の標準語彙でdomainをそのまま表せないかを見る。
...
**[HRA]** stable identity、admitted history、routing policyなどは、意味を所有するpackageを先に決める。
...
```text
HRA.Envelope
HRA.Envelope.Fulfillment
```

**[HRA]** ownershipやprivacyに本物の親子関係ができたときchild packageを検討する。
...
**[HRA]** 次のような値ではprivate typeが自然な候補になる。
...
**[HRA]** containerがprivate typeのrepresentationなら、instantiationもprivate part/body側へ置くと自然なことが多い。
...
**[HRA]** 「このcaseではTargetが存在する」というdomain lawを、commentやdummy valueではなくdata shapeとして表す。
...
**[HRA]** 0がdomain上存在しない座標は、その事実が型に見えるようにする。
...
**[HRA]** containerそのものがpublic contractなら公開してよい。
...
**[HRA]** source declaration order、posting order、evidence orderなど、順序そのものがdomain lawなら、その順序を所有するsequenceを明示する。lookup用mapのiteration orderをdomain orderへ昇格させない。
...
**[HRA]** 「同じalgorithmを複数domain typeへ適用したい」場合は、runtime object hierarchyを作る前にgenericが自然かを見る。
...
**[HRA]** domain ownerを読み取りやすくしたい場所ではqualificationを残し、operator noiseだけ減らしたい場所では`use type`を使う。
...
**[HRA]** call中だけ読むtextは`String`、recordに保持するsource path・metadata・diagnosticは`Unbounded_String`が自然なことが多い。
...
**[HRA]** contractはcaller/program側のlawへ使う。
...
**[HRA]** parser/admissionは、成功なら完全にadmittedなvalue、失敗ならdiagnosticという境界を持つ。
...
**[HRA]** filesystemやstream I/Oのようにoperation自体が失敗する境界では、Adaのexceptionを受けてsource diagnosticへ翻訳することがある。
...
**[HRA]** identity、money、posting、routing、observationはまずvalueとして考える。

### Access type

**[Ada]** aliasing、dynamic allocation、lifetimeを明示して参照する必要がある場合はaccess typeがある。

**[HRA]** その意味がdomainまたはimplementationに本当に存在するとき使う。

### Tagged type / dispatching

**[Ada]** runtime polymorphismが必要ならtagged typeとdispatching operationがある。

**[HRA]** report kindやaccount roleが単なるfinite classificationならenumeration/caseが自然な場合も多い。
...
**[Current]** hraの通常buildはAda 2022、assertions/contracts、warnings、`-O2`を有効にしている。
...
次はAda一般ではなくhraのarchitecture law。
Adaの語彙を使って、これらを明瞭に表現する。

- exact amountは`HRA.Money`が所有する
